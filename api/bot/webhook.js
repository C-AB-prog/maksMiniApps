// api/bot/webhook.js (ESM)
import OpenAI from 'openai';

import { ensureSchema, q, getOrCreateUserIdByTgId } from '../_db.js';
...
export default async function handler(req, res) {
  try {
    ...
    // Telegram часто лучше отвечать 200, чтобы не было бесконечных ретраев
    return res.status(200).json({ ok: true });
  }
}
