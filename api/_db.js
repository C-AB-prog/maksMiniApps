...
const connStr =
  ...
  process.env.POSTGRES_URL ||              // Vercel Storage (pooled) — рекомендуется
  process.env.POSTGRES_URL_NON_POOLING ||  // fallback
  process.env.POSTGRES_PRISMA_URL ||       // fallback
  '';

if (!connStr) throw new Error('DB connection string is missing. Set DATABASE_URL or POSTGRES_URL.');
...
async function ensureSchema() {
  ...
  _ready = (async () => {
    ...
    try {
      ...
      await client.query('ROLLBACK'); _ready = undefined; throw e;
    } finally {
      client.release();
    }
  })();
