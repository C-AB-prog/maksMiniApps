...
module.exports = async (req,res)=>{
  try{
    ...
    if(req.method==='GET'){
      const { rows } = await pool.query(`SELECT id,title,scope,done,due_at,created_at FROM tasks WHERE user_id=$1 ORDER BY created_at DESC`, [auth.user.id]);
    ...
    const { rows } = await pool.query(
      ...
      [auth.user.id, title, scope, due_at]
    );
